# travel.py
ACTIVITY_FILE = "activities.txt"

def load_activities():
    """
    Load activities from a file where each line is in the format: location:activity
    Returns:
        dict: A dictionary where the key is the location and the value is a list of activities.
    """
    activities = {}
    try:
        with open(ACTIVITY_FILE, "r") as file:
            for line in file:
                location, activity = line.strip().split(":", 1)
                if location in activities:
                    activities[location].append(activity)
                else:
                    activities[location] = [activity]
    except FileNotFoundError:
        pass
    return activities

def save_activity(location, activity):
    """
    Save a new activity to the activity file.
    
    Args:
        location (str): The location of the activity.
        activity (str): The activity description.
    """
    with open(ACTIVITY_FILE, "a") as file:
        file.write(f"{location}:{activity}\n")

def swipe_on_activities(username, location, activities):
    """
    Allow the user to swipe on activities for a given location using 'y' for yes and 'n' for no.
    
    Args:
        username (str): The username of the current user.
        location (str): The location to explore activities for.
        activities (list): The list of activities for the location.
    """
    liked_activities = []
    for activity in activities:
        choice = input(f"Do you like this activity: {activity} (y/n)? ")
        if choice.lower() == 'y':
            liked_activities.append(activity)
        elif choice.lower() != 'n':
            print("Invalid input, skipping activity.")

    # Save liked activities to the user's file
    with open(f"{username}_data.txt", "a") as file:
        for liked in liked_activities:
            file.write(f"Liked activity in {location}: {liked}\n")

    print("You liked the following activities:")
    for liked in liked_activities:
        print(f"- {liked}")

def add_activity(location, activities):
    """
    Allow the user to add a new activity to a given location.
    
    Args:
        location (str): The location to add an activity to.
        activities (list): The current list of activities for the location.
    """
    new_activity = input("Enter the new activity: ")
    activities.append(new_activity)
    save_activity(location, new_activity)
    print(f"Added new activity: {new_activity}")

def swipe_and_add_activities(username, activities):
    """
    Allow the user to explore and add activities for a chosen location.
    
    Args:
        username (str): The username of the current user.
        activities (dict): The dictionary containing locations and their respective activities.
    """
    run = True
    while run:
        location = input("Enter the location you want to explore: ").strip()
        if location in activities:
            print(f"Exploring activities for {location}.")
            while True:
                choice = input("Do you want to swipe on activities or add a new activity or exit(swipe/add/exit)? ").lower()
                if choice == "swipe":
                    swipe_on_activities(username, location, activities[location])
                elif choice == "add":
                    add_activity(location, activities[location])
                elif choice == "exit":
                    run = False
                    break
                else:
                    print("Invalid choice.")
        else:
            choice = input(f"No activities found for {location}. Would you like to add one?(y/n)")
            if choice == "y":
                add_activity(location, [])
                choice = input("Do you want to continue?(y/n)")
               
            if choice == "n":
                run = False
                print("Thanks for stopping by. See you soon...")
                break
                
    
                    

